...
export default function AdminOrderDetails() {
  ...
  if (!order)
    return (
      <div className="admin-page">
        <div className="admin-container">Order not found</div>
      </div>
    );
        ...
        <div className="admin-header">
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)' }}>
            ...
            {order.payment_status === 'verification_pending' && (
              ...
              >
                <CheckCircle size={16} style={{ marginRight: '8px' }} /> Verify
                Payment
              </button>
            )}
          </div>
            ...
            <div className="panel">
              ...
              <div className="text-sm text-muted leading-relaxed">
                <p className="font-semibold text-primary mb-1">
                  {order.address?.name || order.address?.full_name}
                </p>
              ...
              {order.payment_method === 'upi' &&
                order.payment_status !== 'paid' && (
                  <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
                    <strong>Admin Note:</strong> Check your bank statement for{' '}
                    <b>{formatPrice(order.total)}</b> from this user before
                    marking as paid.
                  </div>
                )}
            </div>
