...
import { Plus, Edit, Trash2, Save, X, Upload, Download, Search } from 'lucide-react';
...
export default function AdminPincodes() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                showToast(
                    editingId ? 'Pincode updated successfully' : 'Pincode added successfully',
                    'success'
                );
                setIsEditing(false);
                fetchPincodes();
    ...
    const handleBulkUpload = async (e) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                if (response.success) {
                    showToast(`Successfully uploaded ${response.count} pincodes`, 'success');
                    fetchPincodes();
                } else {
    ...
    const handleExportCSV = () => {
        const headers = ['pincode', 'city', 'state', 'is_cod_available', 'estimated_delivery_days', 'is_active'];
        const csvContent = [
            headers.join(','),
            ...pincodes.map(p => [
                p.pincode,
                                        ...
                                        <tbody>
                                            {filteredPincodes.map(pincode => (
                                                <tr key={pincode.id}>
                                                    ...
                                                    <td>
                                                        <span className={`badge ${pincode.is_cod_available ? 'badge--success' : 'badge--secondary'}`}>
                                                            {pincode.is_cod_available ? 'Yes' : 'No'}
                                                        </span>
                                                    </td>
                                                    <td>
                                                        <span className={`badge ${pincode.is_active ? 'badge--success' : 'badge--warning'}`}>
