 ...
 */
const generateUploadParams = (file) => {
...
export const secureUploadMultipleImages = async (files, onProgress) => {
    ...
    const uploadPromises = validFiles.map((file, index) =>
        secureUploadImage(file, (progress) => {
            if (onProgress) {
                const overallProgress = ((index + progress / 100) / validFiles.length) * 100;
 ...
 */
export const generateResponsiveSrcSet = (url, widths = [320, 640, 960, 1280, 1920]) => {
    if (!url || !url.includes('cloudinary.com')) {
        return url;
    }
