/**
 * Shopify-Inspired Inventory Management System
 * Enterprise-grade inventory with reservations and atomic operations
...
/**
 * Reserve inventory for checkout (Shopify-style)
...
/**
 * Complete reservation after successful order
 */
export const completeReservation = async (reservationId, orderId) => {
  try {
...
/**
 * Check real-time available stock (considering reservations)
 */
export const checkAvailableStock = async (productId) => {
  try {
...
export const validateCartStockAsync = async (cartItems) => {
  // Use batch check to avoid N+1 pattern
  ...
  result.data.forEach((check, index) => {
    ...
    if (!check.available) {
      issues.push({
        ...
        issue: check.current_stock === 0 ? 'out_of_stock' : 'insufficient_stock',
        message:
          check.current_stock === 0
            ? `${item.name || check.product_name} is out of stock`
            : `Only ${check.current_stock} available (you have ${item.quantity} in cart)`,
        currentStock: check.current_stock,
