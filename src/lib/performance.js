...
export const initPerformanceMonitoring = () => {
  ...
  if ('PerformanceObserver' in window) {
    try {
      ...
      let clsValue = 0;
      let clsLogCount = 0;
      const MAX_CLS_LOGS = 5; // Only log first 5 CLS events
      const clsObserver = new PerformanceObserver((list) => {
        ...
        // Only log first few CLS events to avoid console spam
        if (clsLogCount < MAX_CLS_LOGS) {
          console.log(`[Web Vital] CLS: ${clsValue.toFixed(3)} (${rating})`);
          clsLogCount++;
          if (clsLogCount === MAX_CLS_LOGS) {
            console.log('[Web Vital] CLS logging stopped to prevent spam');
          }
        }
      ...
      console.warn('Some performance observers not supported:', error.message);
    }
  }
...
export const measureAPICall = async (name, apiCall) => {
  ...
  try {
    ...
    console.error(
      `[API Performance] ${name} failed after ${duration.toFixed(2)}ms`,
      error
    );
    throw error;
  }
